...
def circle_contour(image, contour):
    ...
    print(ellipse) # prints ellipse data such as location on image and size
    #add it
...
def find_orange(image):
    ...
    #cv2.imwrite('test-orange.jpg', image_blur_hsv)


    # Filter by colour
    # 0-10 hue
    #minimum red amount, max red amount
    min_red = np.array([5, 50, 50])
    max_red = np.array([25, 255, 255])
    #layer
    ...
    cv2.imwrite('mask-test.jpg', mask_clean)


    # Find biggest orange
    #get back list of segmented oranges and an outline for the biggest one
    big_orange_contour, mask_oranges = find_biggest_contour(mask_clean)

    # Overlay cleaned mask on image
    # overlay mask on image, orange now segmented
    overlay = overlay_mask(mask_clean, image)

    # Circle biggest orange
...
#read the image
image = cv2.imread('orange-input.jpeg')
#detect it
result = find_orange(image)
#write the new image
cv2.imwrite('orange-output.jpeg', result)
